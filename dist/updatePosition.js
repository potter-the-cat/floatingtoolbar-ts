export function updatePosition(config, state, elements, debug) {
    if (!elements.toolbar)
        return;
    if (!state.isFixed) {
...
function handleFloatingMode(config, state, elements) {
    if (!state.selectionRect || !elements.toolbar)
        return;
...
function handleFixedMode(config, state, elements, debug) {
    if (!elements.toolbar)
        return;
    ...
    if (!hasSelection) {
        ...
        handleFixedModeWithSelection(config, state, elements, contentArea, toolbarContainer, contentWrapper, debug);
    }
}
function resetToFixedPosition(state, elements) {
    if (!elements.toolbar)
        return;
...
function handleFixedModeWithSelection(config, state, elements, contentArea, toolbarContainer, contentWrapper, debug) {
    var _a, _b, _c;
    if (!elements.toolbar || !state.selectionRect)
        return;
    ...
    const measurements = {
        ...
        viewport: {
            ...
            visualTop: ((_a = window.visualViewport) === null || _a === void 0 ? void 0 : _a.offsetTop) || 0,
            visualHeight: ((_b = window.visualViewport) === null || _b === void 0 ? void 0 : _b.height) || window.innerHeight
        },
        contentArea: {
    ...
    const spaceCalculations = {
        ...
        visualViewportHeight: ((_c = window.visualViewport) === null || _c === void 0 ? void 0 : _c.height) || window.innerHeight,
...
export function resetToolbar(state, elements, clearFormatButtonStates) {
