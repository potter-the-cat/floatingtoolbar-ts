...
export function handleRemoveLinkClick(e) {
    ...
    if (this.state.existingLink && this.state.selectionRange) {
        try {
            ...
            (_a = this.state.existingLink.parentNode) === null || _a === void 0 ? void 0 : _a.replaceChild(textNode, this.state.existingLink);
...
export function checkForExistingLink(selection) {
    ...
    // Check if the selection is entirely within a link
    const link = window.findClosestLink(ancestor.nodeType === 3 ? ancestor.parentNode : ancestor);
    if (link) {
        this.debug("Found existing link", link.href);
        return link;
    }
    return null;
}
