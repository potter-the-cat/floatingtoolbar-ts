export function handleSelection(event) {
    var _a;
    const contentArea = document.querySelector(this.config.selector || this.config.content);
    if (!contentArea)
        return;
    ...
    if ('type' in event && event.type === 'mouseup' && contentArea.contains(event.target)) {
        // Store the current width before any changes
        const currentWidth = (_a = this.elements.toolbar) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect().width;
    ...
    if (this.elements.toolbar && 'type' in event && event.type === 'mouseup' &&
        this.elements.toolbar.contains(event.target))
        return;
    ...
    const clickedOutside = 'type' in event && event.type === 'mouseup' &&
        this.elements.toolbar && !this.elements.toolbar.contains(event.target) &&
    ...
    else if (this.elements.toolbar && !this.elements.toolbar.contains(event.target)) {
        this.state.currentSelection = null;
        this.state.selectedText = '';
